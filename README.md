...
โครงการนี้มีเป้าหมายเพื่อวิเคราะห์ปัจจัยที่ทำให้รายได้ของโรงแรม Azure Stay ไม่เติบโต และเสนอแนวทางเพิ่มประสิทธิภาพด้าน Revenue Management โดยมุ่งเน้นการเพิ่มรายได้สุทธิจากการจองผ่านการปรับกลยุทธ์ด้านช่องทางการจอง ราคา ช่วงเวลา และกลุ่มลูกค้า

## Objective Statement

เพิ่มรายได้สุทธิจากการจอง (Net Revenue) อย่างน้อย **12% ภายใน 6 เดือน**  
โดยลดการพึ่งพาช่องทาง OTA ที่มีค่าคอมมิชชั่นสูง และเพิ่มสัดส่วนช่องทางที่สร้างรายได้สุทธิสูงกว่า เช่น Direct Booking และ Walk-in

| SMART Criteria | รายละเอียด |
|---|---|
| **Specific** | เพิ่มรายได้สุทธิจากการจอง โดยเน้นการปรับ Channel Mix, Pricing Strategy และ Customer Segment Strategy |
| **Measurable** | ตั้งเป้าเพิ่ม Net Revenue อย่างน้อย **12%** |
| **Achievable** | ใช้ insight จากข้อมูล เช่น Direct / Walk-in มีรายได้เฉลี่ยสูงกว่า OTA และ Promotion มี trade-off ระหว่าง Demand กับ Revenue |
| **Relevant** | สอดคล้องกับปัญหา Revenue Stagnation และเป้าหมายของธุรกิจในการเพิ่มรายได้และ profitability |
| **Time-bound** | ดำเนินการและประเมินผลภายใน **6 เดือน** |

> **Value Proposition:** เพิ่มรายได้และกำไรสุทธิ โดยไม่จำเป็นต้องเพิ่มจำนวน booking อย่างมีนัยสำคัญ แต่ใช้การบริหารช่องทาง ราคา และกลุ่มลูกค้าให้มีประสิทธิภาพมากขึ้น
...
# 🗂️ 5. Dataset Description And Overview
...
| รายการ | รายละเอียด |
|---|---|
| ประเภทข้อมูล | Transactional Booking Data (ข้อมูลการจองโรงแรมแบบราย booking) |
| จำนวนแถวข้อมูล | 9,515 แถว |
| จำนวนคอลัมน์ | 25 คอลัมน์ (clean) / 29 คอลัมน์ (enriched) |
| ระดับข้อมูล | ข้อมูลอยู่ในระดับราย Booking โดยแต่ละแถวแทนการจอง 1 รายการ ซึ่งประกอบด้วยข้อมูลลูกค้า ห้องพัก ช่องทางการจอง ราคา และรายได้ |
| ช่วงข้อมูล | ครอบคลุมข้อมูลการจองตั้งแต่ ตุลาคม 2024 และช่วงการเข้าพักหลักอยู่ระหว่าง มกราคม 2025 – กุมภาพันธ์ 2025 |

---

## 🔹 Data Structure

โครงสร้างข้อมูลแบ่งออกเป็น 2 ประเภทหลัก ได้แก่:

### 1) Fact Tables (ข้อมูลเชิงธุรกรรม)

| ไฟล์ | รายละเอียด |
|---|---|
| `fact_bookings_clean.csv` | ตารางข้อมูลการจองหลักหลังผ่าน Data Cleaning ใช้เป็นฐานในการวิเคราะห์ |
| `fact_bookings_enriched.csv` | ตารางข้อมูลที่เพิ่มตัวแปรด้านเวลา (Weekend, Holiday, Season) เพื่อใช้ใน Dashboard และ Insight |

### 2) Dimension Tables (ข้อมูลมิติ)

| ไฟล์ | รายละเอียด |
|---|---|
| `dim_channels.csv` | ข้อมูลช่องทางการจอง เช่น Direct, OTA, Walk-in, Corporate |
| `dim_rate_codes.csv` | ข้อมูลประเภทเรทราคา เช่น Rack Rate, Promo, Non-refundable |
| `dim_segments_clean.csv` | ข้อมูลกลุ่มลูกค้า เช่น Leisure, Business, Group, Transient |
| `dim_calendar.csv` | ข้อมูลด้านวัน เช่น Weekday/Weekend, Holiday, Season |
| `dim_room_inventory.csv` | ข้อมูลจำนวนห้องที่เปิดขาย ใช้คำนวณ OCC และ RevPAR |

---

## 🔹 Key Columns Used in Analysis

| คอลัมน์ | คำอธิบาย | การใช้งาน |
|---|---|---|
| `booking_id` | รหัสการจอง | ใช้ระบุ booking และนับจำนวนการจอง |
| `booking_date` | วันที่ทำการจอง | ใช้คำนวณ Booking Lead Time (BLT) |
| `check_in_date` | วันที่เข้าพัก | ใช้คำนวณ ADR, OCC, RevPAR และวิเคราะห์ตามช่วงเวลา |
| `check_out_date` | วันที่ออก | ใช้คำนวณ Length of Stay (LOS) |
| `number_of_rooms` | จำนวนห้องที่จอง | ใช้คำนวณ Room Nights |
| `channel_id` | ช่องทางการจอง | วิเคราะห์ Channel Performance |
| `rate_code_id` | ประเภทเรทราคา | วิเคราะห์ Pricing Strategy |
| `segment_id` | กลุ่มลูกค้า | วิเคราะห์ Customer Segment |
| `True_Room_Revenue` | รายได้จากการเข้าพักจริง | ใช้คำนวณ ADR และ RevPAR |
| `Penalty_Revenue` | รายได้ค่าปรับจาก Cancel/No-show | วิเคราะห์ Revenue Leakage |
| `commission_amount` | คอมมิชชั่น | วิเคราะห์ต้นทุนช่องทาง |
| `net_realized_revenue` | รายได้สุทธิจริง | วิเคราะห์ Profitability |
| `day_type` / `checkin_is_weekend` | ประเภทวัน | วิเคราะห์ Weekday vs Weekend |

---

# 🤖 6. การสร้างข้อมูลด้วย AI (Prompt และ Data Methodology)

ชุดข้อมูลที่ใช้ในโครงการนี้เป็นข้อมูลสังเคราะห์ (Synthetic Data) ที่สร้างขึ้นด้วย Generative AI เพื่อจำลองสถานการณ์การจองโรงแรมของ **Azure Stay** โดยออกแบบให้มีโครงสร้างใกล้เคียงกับข้อมูลจริงจากระบบ Property Management System (PMS) และสอดคล้องกับโจทย์ **Revenue Stagnation**
...
> “สร้างชุดข้อมูลการจองโรงแรมแบบสังเคราะห์ เพื่อใช้วิเคราะห์ปัญหา Revenue Stagnation ของโรงแรม Azure Stay โดยให้มีทั้งตารางข้อมูลหลักและตารางมิติ ได้แก่ `fact_bookings`, `dim_channels`, `dim_rate_codes`, `dim_segments`, `dim_calendar` และ `dim_room_inventory`
>
> ข้อมูลต้องสะท้อน business logic ของโรงแรมจริง เช่น ช่องทาง OTA มีค่าคอมมิชชั่น ช่องทาง Direct และ Walk-in ไม่มีค่าคอมมิชชั่น รายการ No-show ต้องไม่มีรายได้จากการเข้าพักจริง แต่สามารถมีค่าปรับได้ โปรโมชั่นควรช่วยเพิ่มจำนวนการจอง แต่ลดรายได้เฉลี่ยต่อ booking และการจองในช่วง Weekend ควรมีรายได้ต่อ booking สูงกว่า Weekday
>
> ชุดข้อมูลควรสามารถนำไปคำนวณตัวชี้วัดทางธุรกิจ เช่น RevPAR, ADR, Occupancy, LOS, BLT, Net Revenue และ Net Realized Revenue ได้”

---

## 🔹 คำจำกัดความและคำอธิบายของตารางข้อมูล

### `fact_bookings`

ตารางข้อมูลหลักที่เก็บรายละเอียดระดับ booking โดยมีหนึ่งแถวต่อหนึ่งรายการจอง ใช้สำหรับวิเคราะห์รายได้ สถานะการจอง ช่องทางการขาย และพฤติกรรมของลูกค้า
...
ตารางมิติที่อธิบายช่องทางการจอง เช่น Direct, Walk-in, Booking.com, Expedia และ Corporate รวมถึงอัตราค่าคอมมิชชั่นของแต่ละช่องทาง

### `dim_rate_codes`

ตารางมิติที่อธิบายประเภทของราคาและโปรโมชั่น เช่น Rack Rate, Promotion, Non-refundable และ Corporate Rate

### `dim_segments`

ตารางมิติที่อธิบายกลุ่มลูกค้า เช่น Business, Leisure, Group และ Transient

### `dim_calendar`

ตารางมิติด้านเวลา ใช้สำหรับวิเคราะห์รูปแบบตามวัน เช่น Weekday, Weekend และช่วงเวลาอื่น ๆ

### `dim_room_inventory`

ตารางมิติที่เก็บข้อมูลจำนวนห้องพักที่พร้อมขาย ใช้สำหรับคำนวณตัวชี้วัด เช่น RevPAR และ Occupancy

---

## 🔹 คำจำกัดความของตัวชี้วัด (Measures Definition)

| Measure | คำอธิบาย | วิธีการคำนวณ |
|---|---|---|
| **Total_Revenue_THB** | มูลค่ารวมตั้งต้นของ booking ก่อนหักค่าคอมมิชชั่น | มูลค่ารวมของ booking |
| **Commission_THB** | ค่าคอมมิชชั่นที่โรงแรมต้องจ่ายให้ช่องทางการจอง เช่น OTA | `Total_Revenue_THB × commission_rate` |
| **Net_Revenue_THB** | รายได้สุทธิหลังหักค่าคอมมิชชั่น | `Total_Revenue_THB - Commission_THB` |
| **True_Room_Revenue** | รายได้จากการเข้าพักจริงของลูกค้า โดยไม่รวมกรณี No-show | ขึ้นอยู่กับสถานะ booking |
| **Penalty_Revenue** | รายได้จากค่าปรับที่เกิดจากกรณี Cancel หรือ No-show | ขึ้นอยู่กับ business rule |
| **net_realized_revenue** | รายได้ที่เกิดขึ้นจริงตามสถานะ booking โดยรวมรายได้จากการเข้าพักจริงและค่าปรับ | `True_Room_Revenue + Penalty_Revenue` |
| **RevPAR** | รายได้ต่อจำนวนห้องที่สามารถขายได้ ใช้วัดประสิทธิภาพการสร้างรายได้ของโรงแรม | `Total Room Revenue / Total Rooms Available` |
| **ADR** | รายได้เฉลี่ยต่อห้องที่ขายได้ | `Total Room Revenue / Number of Rooms Sold` |
| **Occupancy Rate (OCC)** | สัดส่วนห้องที่ถูกขายเมื่อเทียบกับจำนวนห้องที่พร้อมขาย | `(Number of Rooms Sold / Total Rooms Available) × 100` |
| **LOS** | จำนวนคืนที่ลูกค้าเข้าพัก | `Check-out Date - Check-in Date` |
| **BLT** | จำนวนวันระหว่างวันที่จองกับวันที่เข้าพัก | `Check-in Date - Booking Date` |

---

## 🔹 ตัวแปรมิติ (Dimensions)

| Dimension | คำอธิบาย | ตัวอย่างค่า |
|---|---|---|
| **channel_id** | ช่องทางการจอง | `CH_DIR`, `CH_WLK`, `CH_BKG`, `CH_EXP`, `CH_COR` |
| **rate_code_id** | ประเภทของราคา / โปรโมชั่น | `RC_RACK`, `RC_PROMO`, `RC_NONREF`, `RC_CORP` |
| **segment_id** | กลุ่มลูกค้า | `SEG_LEI`, `SEG_TRA`, `SEG_GRP`, `SEG_BUS` |
| **day_type** | ประเภทของวัน | `Weekday`, `Weekend` |
| **status** | สถานะของ booking | `Checked-Out`, `Cancelled`, `No-Show` |

---

## 🔹 ตัวแปรที่ได้มาจากเงื่อนไข (Derived Variables / Conditional Logic)

| Variable | Logic |
|---|---|
| **day_type** | หากวันเข้าพักเป็นวันเสาร์หรือวันอาทิตย์ → `Weekend` หากเป็นวันจันทร์ถึงวันศุกร์ → `Weekday` |
| **LOS** | `check_out_date - check_in_date` |
| **BLT** | `check_in_date - booking_date` |
| **Commission_THB** | คำนวณจาก `Total_Revenue_THB × commission_rate` เฉพาะช่องทางที่มีค่าคอมมิชชั่น |
| **Net_Revenue_THB** | `Total_Revenue_THB - Commission_THB` |
| **True_Room_Revenue** | หาก `status = Checked-Out` จะนับเป็นรายได้จากการเข้าพักจริง หาก `status = No-Show` จะเป็น `0` |
| **Penalty_Revenue** | หาก `status = No-Show` หรือ `Cancelled` และมีค่าปรับ จะบันทึกเป็นรายได้จาก penalty |
| **net_realized_revenue** | `True_Room_Revenue + Penalty_Revenue` |

---

## 🔹 No-show Business Logic

ในกรณีที่ booking มีสถานะเป็น **No-Show** จะถือว่าไม่มีการเข้าพักจริงเกิดขึ้น ดังนั้น:

- `True_Room_Revenue = 0`
- `Penalty_Revenue` อาจมีค่าได้ หากโรงแรมสามารถเรียกเก็บค่าปรับ
- `net_realized_revenue` ใช้สะท้อนรายได้ที่เกิดขึ้นจริงจาก booking นั้นตามสถานะที่เกิดขึ้น

> หมายเหตุ: `Net_Revenue_THB` ใช้สำหรับดูรายได้หลังหักค่าคอมมิชชั่น ส่วน `net_realized_revenue` ใช้สำหรับดูรายได้ที่เกิดขึ้นจริงตามสถานะของ booking
